...
class ProductModel{
    ...
    function getMarca($idMarca) {
        $query = $this->db->prepare('SELECT Nombre, Precio, Img FROM productos WHERE ID_Marca = ?');
    ...
    function getCategoria($categoria) {
        $query = $this->db->prepare('SELECT Nombre, Precio, Img FROM  productos WHERE Categoria = ?');
    ...
    function getPrecioAsc() {
        $query = $this->db->prepare('SELECT Nombre, Precio FROM productos ORDER BY Precio');
        $query->execute();
        return $query->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    function getPrecioDesc() {
        $query = $this->db->prepare('SELECT Nombre, Precio FROM productos ORDER BY Precio DESC');
        $query->execute();
        return $query->fetchAll(PDO::FETCH_OBJ);
    }
    //listado segun orden nombre asc
    function getNombreAsc() {
        $query = $this->db->prepare('SELECT Nombre, Precio FROM productos ORDER BY Nombre ');
        $query->execute();
        return $query->fetchAll(PDO::FETCH_OBJ);
    }
    //listado segun orden nombre desc
    function getNombreDesc() {
        $query = $this->db->prepare('SELECT Nombre, Precio FROM productos ORDER BY Nombre DESC ');
        $query->execute();
        return $query->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    function instert($nombre, $descripcion , $precio , $peso , $categoria , $idMarca){
        $query = $this->db -> prepare('INSERT INTO productos (Nombre, Descripcion , Precio , Peso , Categoria , ID_Marca ) VALUES (?, ?, ?, ?, ?, ?)');
        $query -> execute([$nombre, $descripcion , $precio , $peso , $categoria , $idMarca]);

        /* Obtengo y devuelvo el id del ultimo insert */
